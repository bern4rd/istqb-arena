...
export default function SimulationConfig({ certification, language, onCancel, onStartSimulation }: SimulationConfigProps) {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-3xs flex items-center justify-center p-4 z-50 overflow-y-auto font-sans">
      <div className="bg-white rounded-xl shadow-lg max-w-2xl w-full border border-slate-200 overflow-hidden transform scale-100 transition-all">
        
        {/* Banner with Title */}
        <div className="bg-slate-900 text-white p-6 relative">
          <GraduationCap className="absolute right-6 top-6 w-14 h-14 text-slate-800 opacity-60" />
          ...
          <p className="text-[11px] text-slate-400 mt-2 flex items-center gap-4">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="inline-flex py-0.5 px-2 rounded bg-blue-100 text-blue-700 font-mono text-[9px] font-bold uppercase tracking-wider">
                  {t.trainingMode}
                </span>
                {selectedMode === "training" && (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="inline-flex py-0.5 px-2 rounded bg-slate-900 text-white font-mono text-[9px] font-bold uppercase tracking-wider">
                  {t.examMode}
                </span>
                {selectedMode === "exam" && (
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 text-[11px] text-slate-600 flex items-start gap-2.5">
            <BookOpen className="w-4 h-4 text-slate-500 shrink-0 mt-0.5" />
            <div>
              <span className="font-bold text-slate-955 block mb-0.5">
        ...
        <div className="bg-slate-50 border-t border-slate-100 p-4 flex justify-end gap-3">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 text-xs font-semibold text-slate-600 hover:text-slate-950 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
          >
